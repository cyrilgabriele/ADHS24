public class HS21_DP {
    ...
    public static int dpAlgo(int n, int A, int B, int[] array) {
        ...
        for (int i = 1; i <= n; i++) {          // Iterate over elements
            for (int j = 0; j <= A; j++) {      // Iterate over all possible sums
                for (int k = 0; k <= B; k++) {  // Iterate over all possible sum-of-squares
        ...
        return dp[n][A][B]; // Result: Can we form subset with sum A and sum-of-squares B?
    }
}
